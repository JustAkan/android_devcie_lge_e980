...
static void datacb_intercept(int32_t msg,
                      const camera_memory_t *data, unsigned int index,
                      camera_frame_metadata_t *metadata,
                      void *cookie) {
    if (data != NULL)
        sDataCb(msg, data, index, metadata, cookie);
}
        ...
        void *user)
{
    ...
    sDataCb = data_cb;
    VENDOR_CALL(device, set_callbacks, notify_cb, datacb_intercept, data_cb_timestamp, get_memory, user);
}
